  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-40 bg-[#002FA7] text-white p-4 hover:bg-[#001F70] transition-all no-print"
        style={{
        ...
        <div className="relative">
          ...
          {!open && (
            <div
              ...
              style={{ animation: "pulse-dot 1.5s ease-in-out infinite", boxShadow: "0 0 0 2px #002FA7" }}
            />
          )}
        </div>
      </button>
      ...
      {open && (
        ...
        >
          {/* Header */}
          <div className="relative overflow-hidden bg-[#002FA7] text-white">
          ...
          <div ref={boxRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-[#FAFAFA]" data-testid="ai-messages">
            {messages.map((m, i) => (
              <div key={i} className={`max-w-[88%] ${m.role === "user" ? "ml-auto" : ""} fade-up`} style={{ animationDelay: `${i * 30}ms` }}>
                <div className={`text-[9px] font-mono tracking-widest uppercase font-semibold mb-1.5 flex items-center gap-1.5 ${m.role === "user" ? "justify-end text-[#002FA7]" : "text-[#5C5C5C]"}`}>
                  {m.role === "user" ? "YOU" : (
                    <>
            ...
            {loading && (
              <div className="max-w-[88%] fade-up">
                ...
                <div className="text-sm border border-[#E5E5E5] p-3 bg-white flex items-center gap-1">
                  <div className="w-1.5 h-1.5 bg-[#002FA7] rounded-full" style={{ animation: "pulse-dot 0.9s ease-in-out infinite" }} />
                  <div className="w-1.5 h-1.5 bg-[#002FA7] rounded-full" style={{ animation: "pulse-dot 0.9s ease-in-out infinite", animationDelay: "0.15s" }} />
                  <div className="w-1.5 h-1.5 bg-[#002FA7] rounded-full" style={{ animation: "pulse-dot 0.9s ease-in-out infinite", animationDelay: "0.3s" }} />
                </div>
              </div>
            )}
                ...
                <div className="flex flex-wrap gap-2">
                  {SUGGESTIONS.map((s) => (
                    <button
                      key={s}
                      onClick={() => send(s)}
                      className="text-xs px-3 py-1.5 border border-[#E5E5E5] hover:border-[#002FA7] hover:text-[#002FA7] bg-white transition flex items-center gap-1.5"
                    >
                      <Lightning size={10} /> {s}
                    </button>
                  ))}

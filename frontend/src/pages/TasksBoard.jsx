...
const LANES = [
  ...
  { id: "in_progress", label: "In progress", color: "#8B7F6A" },
...
const STATUS_COLORS = {
  ...
  "Ordered": "#0EA5E9", "Work Started": "#0891B2", "In Progress": "#8B7F6A",
...
export default function TasksBoard() {
  ...
  const loadAll = async () => {
    const [t, p, m, v, e] = await Promise.all([
      api.get("/tasks"),
      api.get("/projects"),
      api.get("/tasks/meta"),
  ...
  const bulkApply = async (patch) => {
    ...
    try {
      await api.post("/tasks/bulk-update", { task_ids: [...selected], ...clean });
      toast.success(`Updated ${selected.size} task${selected.size > 1 ? "s" : ""}`);
      setSelected(new Set());
      loadAll();
    } catch (ex) {
      toast.error(ex?.response?.data?.detail || "Bulk update failed");
    }
  };
  ...
  return (
    <div className="space-y-6" data-testid="tasks-page">
      ...
      <div className="flex flex-wrap items-center gap-2" data-testid="task-filters">
        ...
        <select className="input-flat !w-auto min-w-[150px]" value={filters.project_id} onChange={(e) => setFilters({ ...filters, project_id: e.target.value })} data-testid="filter-project">
          <option value="">All projects</option>
          {projects.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
        </select>
        <select className="input-flat !w-auto min-w-[120px]" value={filters.area} onChange={(e) => setFilters({ ...filters, area: e.target.value })} data-testid="filter-area">
          <option value="">All areas</option>
          {areas.map((a) => <option key={a}>{a}</option>)}
        </select>
        <select className="input-flat !w-auto min-w-[130px]" value={filters.category} onChange={(e) => setFilters({ ...filters, category: e.target.value })} data-testid="filter-category">
          <option value="">All categories</option>
          {categories.map((c) => <option key={c}>{c}</option>)}
        </select>
        <select className="input-flat !w-auto min-w-[120px]" value={filters.priority} onChange={(e) => setFilters({ ...filters, priority: e.target.value })} data-testid="filter-priority">
          <option value="">All priorities</option>
          {(meta?.priorities || []).map((p) => <option key={p}>{p}</option>)}
        </select>
        <select className="input-flat !w-auto min-w-[120px]" value={filters.status_detail} onChange={(e) => setFilters({ ...filters, status_detail: e.target.value })} data-testid="filter-status">
...
// ================================================================
// Bulk actions bar — status, assignee, due date, priority in one go
// ================================================================
function BulkBar({ count, meta, employees, onApply, onClear }) {
  const [patch, setPatch] = useState({ status_detail: "", priority: "", assignee_id: "", due_date: "" });

  const apply = () => {
    const emp = employees.find((e) => e.id === patch.assignee_id);
    onApply({
  ...
  return (
    <div className="border border-[#0A0A0A] bg-[#0A0A0A] text-white px-4 py-3 scale-in space-y-2" data-testid="bulk-bar">
      <div className="flex flex-wrap items-center gap-3 text-sm">
        <ListChecks size={16} />
        <span className="font-mono text-xs tracking-wider">{count} SELECTED</span>
        <select value={patch.status_detail} onChange={(e) => setPatch({ ...patch, status_detail: e.target.value })}
          className="bg-transparent border border-white/30 px-2 py-1.5 text-xs" data-testid="bulk-status">
          <option value="" className="text-black">Status — no change</option>
          {(meta?.status_detail || []).map((s) => <option key={s} className="text-black">{s}</option>)}
        </select>
        <select value={patch.priority} onChange={(e) => setPatch({ ...patch, priority: e.target.value })}
          className="bg-transparent border border-white/30 px-2 py-1.5 text-xs" data-testid="bulk-priority">
          <option value="" className="text-black">Priority — no change</option>
          {(meta?.priorities || []).map((p) => <option key={p} className="text-black">{p}</option>)}
        </select>
        <select value={patch.assignee_id} onChange={(e) => setPatch({ ...patch, assignee_id: e.target.value })}
        ...
        <label className="flex items-center gap-1.5 text-xs">
          <span className="opacity-70 font-mono uppercase">Due</span>
          <input type="date" value={patch.due_date} onChange={(e) => setPatch({ ...patch, due_date: e.target.value })}
            className="bg-transparent border border-white/30 px-2 py-1 text-xs [color-scheme:dark]" data-testid="bulk-due-date" />
        </label>
        <button onClick={apply} className="bg-white text-[#0A0A0A] px-4 py-1.5 text-xs font-semibold hover:bg-[#F0F0F0] transition"
          data-testid="bulk-apply">
          Apply to {count}
        </button>
        <button onClick={onClear} className="ml-auto text-xs opacity-70 hover:opacity-100" data-testid="bulk-clear">
          <X size={12} className="inline mr-1" />Clear selection
        </button>
      </div>
    </div>
  );
...
// Kanban view
// ================================================================
function KanbanView({ tasks, onDelete, onDragStart, onDrop, dragOver, setDragOver, onOpen, selected, onToggleSelect }) {
  const today = new Date().toISOString().slice(0, 10);
  return (
              ...
              {items.map((t) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => onOpen(t.id)}
                    className={`border p-3 bg-white cursor-grab active:cursor-grabbing transition-all hover:-translate-y-0.5 group ${
                      isSel ? "border-[#8B7F6A] ring-1 ring-[#8B7F6A]" :
                      overdue ? "border-[#FF2A00]" : "border-[#E5E5E5] hover:border-[#0A0A0A]"}`}
                  ...
                  >
                    <div className="flex items-center gap-2 mb-2 text-[10px] font-mono uppercase tracking-wider">
                      <button
                        ...
                        title={isSel ? "Deselect" : "Select for bulk actions"}
                        className={`transition ${isSel ? "text-[#8B7F6A]" : "text-[#CCCCCC] hover:text-[#0A0A0A]"}`}
                        data-testid={`select-${t.id}`}
                      >
                        {isSel ? <CheckSquare size={14} weight="fill" /> : <Square size={14} />}
                      </button>
                      <span className="px-1.5 py-0.5 border" style={{ borderColor: isVendor ? "#8A6DFF" : "#8B7F6A", color: isVendor ? "#8A6DFF" : "#8B7F6A" }}>
                    ...
                    <div className="flex items-center justify-between mt-3 pt-3 border-t border-[#F0F0F0]">
                      <div className="text-[10px] font-mono tracking-wider uppercase text-[#5C5C5C]">
                        ...
                        {(t.assignee_name || t.vendor_contact?.vendor_name) && (
                          <span className="ml-2 text-[#0A0A0A]">· {t.task_type === "vendor" ? t.vendor_contact?.vendor_name : t.assignee_name}</span>
                        )}
                      </div>
          ...
          {tasks.map((t, i) => {
            ...
            return (
              <tr key={t.id} className={`border-b border-[#F0F0F0] hover:bg-[#FAFAFA] ${overdue ? "bg-[#FFF2F0]" : ""}`}
                ...
                <td className="p-2">
                  <button onClick={() => onOpen(t.id)} className="text-left font-semibold hover:text-[#8B7F6A]" data-testid={`open-${t.id}`}>
                    {t.title}
                  </button>

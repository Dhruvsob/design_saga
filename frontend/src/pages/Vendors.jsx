  ...
  return (
    <div className="space-y-6" data-testid="vendors-page">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="flex items-center gap-2 flex-1 max-w-md px-3 py-2 border border-[#E5E5E5] focus-within:border-[#8B7F6A] transition">
        ...
        <div className="overflow-x-auto"><table className="w-full">
          ...
          <tbody>
            ...
            {rows.map((v) => (
              ...
              >
                <Td>
                  <div className="font-semibold group-hover:text-[#8B7F6A]">
                ...
                <Td className="text-right">
                  <Link to={`/vendors/${v.id}`} className="text-[#8B7F6A] text-xs font-semibold flex items-center gap-1 justify-end">
...
function Kpi({ label, value, accent, testid }) {
  return (
    <div className={`card-flat ${accent ? "ring-1 ring-[#8B7F6A]/20" : ""}`} data-testid={testid}>
      <div className="overline">{label}</div>
      <div className={`font-display font-bold text-2xl mt-1 ${accent ? "text-[#8B7F6A]" : ""}`}>{value}</div>
    </div>
  );
}
...
function TypePill({ type }) {
  const map = {
    ...
    supplier: "bg-[#EEF2FF] text-[#8B7F6A]",

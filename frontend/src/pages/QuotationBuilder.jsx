...
export default function QuotationBuilder() {
  ...
  return (
    <div className="space-y-6 pb-32" data-testid="quotation-builder">
      ...
      <div className="border border-[#E5E5E5] p-6 flex flex-wrap gap-6 items-start justify-between bg-white">
        <div className="flex-1 min-w-[280px]">
          ...
          <input
            ...
            className="font-display font-bold tracking-tight text-3xl w-full bg-transparent outline-none border-b border-transparent focus:border-[#002FA7]"
      ...
      {/* Tab content */}
      <div>
        {tab === "Overview" && <Overview q={q} update={update} tpl={tpl} />}
        {tab === "BOQ Builder" && <BoqBuilder q={q} update={update} tpl={tpl} />}
        {tab === "Rooms" && <Rooms q={q} />}
        {tab === "Materials" && <Materials q={q} update={update} tpl={tpl} />}
        {tab === "Costing" && <Costing q={q} update={update} />}
        {tab === "Payment Plan" && <PaymentPlan q={q} update={update} tpl={tpl} />}
        {tab === "Timeline" && <Timeline q={q} update={update} tpl={tpl} />}
        {tab === "Terms" && <Terms q={q} update={update} tpl={tpl} />}
        {tab === "Versions" && <Versions q={q} />}
        {tab === "Preview" && <Preview qid={id} />}
      </div>
...
/* ========= Overview ========= */
function Overview({ q, update, tpl }) {
  ...
  const swapType = (newType) => {
    if (!window.confirm(`Switch type to ${TYPE_LABELS[newType]}? Default payment plan, timeline and deliverables will reset.`)) return;
    update({
      type: newType,
          ...
          <div className="grid grid-cols-2 gap-2">
            {Object.entries(TYPE_LABELS).map(([v, l]) => (
              <button
                key={v}
                onClick={() => swapType(v)}
                className={`p-3 border text-left transition ${q.type === v ? "bg-[#002FA7] border-[#002FA7] text-white" : "bg-white border-[#E5E5E5] hover:border-[#0A0A0A]"}`}
              >
...
/* ========= BOQ Builder ========= */
function BoqBuilder({ q, update, tpl }) {
  const [showCatForm, setShowCatForm] = useState(false);
  const [newCat, setNewCat] = useState("");

  const addTemplateCat = (name) => {
    const items = (tpl.boq_templates[name] || []).map((it) => ({
      ...
      rate: it.rate, margin_pct: it.margin_pct, amount: 0,
      room: tpl.rooms[0], brand_tier: "Standard", vendor: "",
    }));
  ...
  const addItem = (ci) => {
    ...
    boq[ci].items.push({
      ...
      description: "", unit: "nos", quantity: 1, rate: 0, margin_pct: 15,
      amount: 0, room: tpl.rooms[0], brand_tier: "Standard", vendor: "",
    });
    update({ boq });
  };
  ...
  return (
    <div className="space-y-6" data-testid="tab-boq">
      <Card title="ADD FROM LIBRARY">
        <div className="flex flex-wrap gap-2">
          {Object.keys(tpl.boq_templates).map((name) => (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left border-b border-[#E5E5E5]">
                  ...
                  <th className="py-2 overline" style={{ minWidth: 220 }}>Description</th>
                  ...
                  <th className="py-2 overline text-right">Mgn%</th>
                  <th className="py-2 overline text-right">Amount</th>
                  <th className="py-2 overline">Room</th>
                  <th className="py-2 overline">Brand</th>
                ...
                {cat.items.map((it, ii) => {
                  ...
                  return (
                    <tr key={ii} className="border-b border-[#F0F0F0]">
                      ...
                      <td><input type="number" className="input-flat text-right" style={{ padding: "6px 8px", width: 70 }} value={it.margin_pct} onChange={(e) => updateItem(ci, ii, "margin_pct", Number(e.target.value) || 0)} /></td>
      ...
      {(!q.boq || q.boq.length === 0) && (
        <Card title="EMPTY BOQ">
          <p className="text-sm text-[#5C5C5C]">Add a category from the library above to get started.</p>
        </Card>
      )}
    </div>
  );
        ...
        {(q.timeline || []).map((p, i) => (
          <div key={i} className="grid grid-cols-12 gap-3 items-center">
            ...
            <div className="col-span-2 h-6 bg-[#F0F0F0] relative">
              <div className="absolute top-0 h-6 bg-[#002FA7]" style={{ left: `${(p.start_offset_weeks / Math.max(total, 1)) * 100}%`, width: `${(p.duration_weeks / Math.max(total, 1)) * 100}%` }} />
            </div>
      ...
      <div className="space-y-4">
        {(q.terms || []).map((t, i) => (
          <div key={i} className="border border-[#E5E5E5] p-4">
            <div className="flex items-center justify-between mb-2">
              <input className="font-display font-bold text-lg bg-transparent outline-none border-b border-transparent focus:border-[#002FA7]" value={t.section || ""} onChange={(e) => setRow(i, "section", e.target.value)} />
              <button onClick={() => removeRow(i)} className="text-[#FF2A00]"><Trash size={14} /></button>
            </div>

...
async def tasks_meta(request: Request,
    ...
    if project_id:
        ...
        p = await db.projects.find_one({"id": project_id}, {"_id": 0, "custom_areas": 1, "custom_categories": 1})
        if p:
...
async def list_tasks(request: Request,
    ...
    rows = await db.tasks.find(q, {"_id": 0}).sort("created_at", -1).to_list(2000)
    return rows
...
async def list_project_areas(project_id: str, request: Request,
                             ...
                             authorization: Optional[str] = Header(default=None)):
    await require_user(request, session_token, authorization)
    p = await db.projects.find_one({"id": project_id}, {"_id": 0, "custom_areas": 1})
...
async def list_project_categories(project_id: str, request: Request,
                                  ...
                                  authorization: Optional[str] = Header(default=None)):
    await require_user(request, session_token, authorization)
    p = await db.projects.find_one({"id": project_id}, {"_id": 0, "custom_categories": 1})
